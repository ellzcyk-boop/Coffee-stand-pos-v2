...
function renderCustomers(){
 ...
 const card=document.createElement("div");card.className="customer-card";card.innerHTML=`<div class="row"><div><strong>${esc(c.name)}</strong><div class="muted">${c.purchases.length} purchase${c.purchases.length===1?"":"s"} · Weekend ${money(customerTotal(c))}</div></div><div class="right"><span class="badge ${customerDue(c)?"due":""}">${customerDue(c)?`Due ${money(customerDue(c))}`:"Paid"}</span></div></div><div class="toolbar"><button class="primary">+ New Purchase</button><button>History</button></div><div class="history hidden"></div>`;card.querySelector("button").onclick=()=>startFor(c.name);card.querySelectorAll("button")[1].onclick=()=>history(card,c);list.appendChild(card)}});
