...
function renderCustomers(){
 ...
 const card=document.createElement("div");card.className="customer-card";card.innerHTML=`<div class="row"><div><strong>${esc(c.name)}</strong><div class="muted">${c.purchases.length} purchase${c.purchases.length===1?"":"s"} · Weekend ${money(customerTotal(c))}</div></div><div class="right"><span class="badge ${customerDue(c)?"due":""}">${customerDue(c)?`Due ${money(customerDue(c))}`:"Paid"}</span></div></div><div class="toolbar"><button class="primary">+ New Purchase</button><button>History</button><button class="paid-all" ${customerDue(c)?"":"disabled"}>Mark All Paid</button><button class="danger delete-customer" type="button">Delete Customer</button></div><div class="history hidden"></div>`;card.querySelector("button").onclick=()=>startFor(c.name);card.querySelectorAll("button")[1].onclick=()=>history(card,c);const paidAll=card.querySelector(".paid-all");if(paidAll)paidAll.onclick=async()=>{if(!confirm(`Mark all purchases for ${c.name} as paid?`))return;c.purchases.forEach(p=>p.paid=true);await save();renderAll()};
 const deleteBtn=card.querySelector(".delete-customer");if(deleteBtn)deleteBtn.onclick=async()=>{if(!confirm(`Delete ${c.name} and all of their purchases? This cannot be undone unless you have a backup.`))return;state.customers=state.customers.filter(x=>x.id!==c.id);if(currentName.toLowerCase()===c.name.toLowerCase()){current=[];currentName="";$('customerName').value="";renderOrder()}await save();renderAll()};
